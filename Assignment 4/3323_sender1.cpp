...
int main(){
    ...
    while (zx1--){ 
        for(int x1=x1i;x1<pack.size();x1++){
            char inp[5]={ pack[x1] ,'\0'} ;
            send(recvSock, inp, 2,0) ;             // sending message to reciever
            cout << "sent"<<x1<<"\n" ;
            char recvbuf[5] ;
            int inLen;
            if( (inLen = recv(recvSock, recvbuf, 5, 0)) <0){       // accepting response from reciever
        ...
        char recvbuf[5] ;
            ...
            if(recvbuf[0]!='-'){                                        // displaying message correspnding the response from reciever
                ...
                x1i = recvbuf[0]-'0' ;
                continue ;
            } else { cout << "------- Packets recieved successfully !! -------" ; break ; }
            cout << endl;
    }
            cout << endl<< endl << endl;
